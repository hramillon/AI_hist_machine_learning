...
class SwissRoll(ThreeDScene):
    def construct(self):
        ...
        def make_pts(alpha_z=1.0, alpha_unroll=0.0):
            ...
            pts[:, 0] = (pts[:, 0] - pts[:, 0].mean()) / pts[:, 0].std() * 2.5
            pts[:, 2] = (pts[:, 2] - pts[:, 2].mean()) / (pts[:, 2].std() + 1e-6) * 2.5 * alpha_z
            return pts
        ...
        # ── 2. Projection : z → 0 + caméra du dessus ────────────────────────
        ...
        # ── 3. Retour vue 3D puis déroulage ─────────────────────────────────
        ...
        # Remettre les points en 3D d'abord
        self.play(
